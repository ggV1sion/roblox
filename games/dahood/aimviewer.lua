...
LockDetector.Material = _G.Settings.Material
LockDetector.Color = _G.Settings.Color
LockDetector.Transparency = _G.Settings.Transparency

local function isLocalPlayer(d)
    if _G.Settings.LocalPlayer then
        return true
    else
...
local function getClosestPlayerToCursor()
    ...
    for _, player in next, Services.Players:GetPlayers() do
        if c(player) and player.Character and player.Character:FindFirstChild("Humanoid") and
            player.Character.Humanoid.Health ~= 0 and player.Character:FindFirstChild(_G.Settings.Part) then
...
Local.Mouse.KeyDown:Connect(function(key)
    if key == _G.Settings.Keybind then
        if enabled then
            enabled = false
            player = nil
...
Services.RunService.Stepped:Connect(function()
    if player and player.Character:FindFirstChildWhichIsA("Tool") and
        ...
        Services.Players:FindFirstChild(tostring(player)) and player.Character:FindFirstChild(_G.Settings.Part) then
        local distance = (player.Character[_G.Settings.Part].CFrame.Position -
                             player.Character.BodyEffects.MousePos.Value).Magnitude
        local mid = (player.Character[_G.Settings.Part].CFrame.Position + player.Character.BodyEffects.MousePos.Value) /
                        2
        LockDetector.Size = Vector3.new(_G.Settings.Size[1], _G.Settings.Size[2], distance)
